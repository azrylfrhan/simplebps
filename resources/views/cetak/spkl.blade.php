...
<body>
    <div class="container-surat">
        <div class="d-flex align-items-center mb-4">
            <img src="{{ asset('image/logoBps.png') }}" alt="Logo BPS" style="width: 80px;" class="me-3">
            <div>
        ...
        <div class="text-center mb-4">
            ...
            <p class="mb-0"><b>Nomor : {{ $nomorSpkl }}</b></p>
        </div>
    ...
    <tbody>
        @php $no = 1; @endphp
        @foreach($grouped as $nama => $item)
            @php $rowCount = count($item['data_lembur']); @endphp
            @foreach($item['data_lembur'] as $maksud => $tanggalList)
                <tr>
                    @if($loop->first)
                        <td rowspan="{{ $rowCount }}" style="text-align: center; vertical-align: middle;">{{ $no++ }}</td>
                        <td rowspan="{{ $rowCount }}" style="vertical-align: middle;">{{ $nama }}</td>
                        <td rowspan="{{ $rowCount }}" style="vertical-align: middle;">{{ $item['jabatan'] }}</td>
                    @endif
                    <td style="text-align: center; vertical-align: middle;">
                        {{ implode(', ', $tanggalList) }} {{ \Carbon\Carbon::create()->month($bulan)->locale('id')->translatedFormat('F') }} {{ $tahun }}
                    </td>
                    <td style="vertical-align: middle;">- {{ $maksud }}</td>
                </tr>
            @endforeach
        @endforeach
    </tbody>
        ...
        <p>Untuk menyelesaikan pekerjaan tersebut dengan pengawasan penuh dari Kepala Bagian Umum, segala biaya yang timbul berkenaan dengan pelaksanaan kerja lembur tersebut dibebankan pada DIPA BPS Provinsi Sulawesi Utara Tahun Anggaran {{ $tahun }}.</p>
