...
<div class="container-fluid py-4">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="badge px-4 py-2 rounded-pill shadow-sm" style="background-color: rgba(0, 74, 105, 0.1); color: #004a69;">
            <i class="fas fa-hourglass-half me-2"></i> {{ count($pengajuan) }} Menunggu Tindakan
        </div>
    </div>
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-muted x-small fw-bold">{{ $p->pegawai->tim->nama_tim ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="text-center">
                                <div class="date-badge">
                                    <div class="date-day">{{ \Carbon\Carbon::parse($p->tanggal)->format('d') }}</div>
                                    <div class="date-month">{{ \Carbon\Carbon::parse($p->tanggal)->locale('id')->translatedFormat('M Y') }}</div>
                                </div>
                            </td>
                            <td class="text-center">
                                <span class="badge rounded-pill px-3 py-2" style="background-color: #e6f0f5; color: #004a69;">
                                    {{ $p->lama_jam_taksiran }} Jam
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="text-end pe-4">
                                <div class="btn-group rounded-3 overflow-hidden border shadow-sm">
                                    <button class="btn btn-action btn-sm px-3 border-end" data-bs-toggle="modal" data-bs-target="#accModal{{ $p->id }}" title="Setujui">
                                        <i class="fas fa-check" style="color: #004a69;"></i>
                                    </button>
                                    <button class="btn btn-action btn-sm px-3" data-bs-toggle="modal" data-bs-target="#tolakModal{{ $p->id }}" title="Tolak">
...
@foreach($pengajuan as $p)
    <div class="modal fade" id="accModal{{ $p->id }}" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-content border-0" style="border-radius: 15px;">
                    ...
                    <div class="modal-body p-4">
                        <div class="d-flex align-items-center mb-4 p-3 rounded-3" style="background-color: #f0f4f7;">
                            <div class="avatar-box me-3">{{ strtoupper(substr($p->pegawai->nama_lengkap, 0, 1)) }}</div>
                            <div>
                                <h6 class="mb-0 fw-bold text-dark">{{ $p->pegawai->nama_lengkap }}</h6>
                                <small class="text-muted">Diajukan: {{ \Carbon\Carbon::parse($p->tanggal)->translatedFormat('d F Y') }}</small>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" name="lama_jam_disetujui" class="form-control form-control-lg fw-bold" value="{{ $p->lama_jam_taksiran }}" style="color: #004a69;" required>
